...
1. **macOS adhoc code signing only.** Releases ship with an adhoc signature; first-run Gatekeeper on a fresh machine requires a right-click → Open dance OR `xattr -dr com.apple.quarantine` (the Homebrew cask handles this automatically because cask-downloaded apps aren't quarantined in the first place). Apple Developer ID notarization is tracked; held until usage justifies the $99/yr + setup overhead.
