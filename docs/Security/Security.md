...
| macOS code signing         | Releases are Developer ID-signed (Smoo LLC), notarized and stapled; updates are Sparkle 2 with EdDSA-signed archives. See [Sparkle Updates](../Operations/Sparkle-Updates.md). |
...
- **For drafts you haven't posted:** compose drafts (text, reply/quote target, and the *paths* of attached media — not the media bytes) are stored in the local app database (`~/Library/Application Support/ai.Smoo.smooblue/smooblue.db`, `drafts` table) so work in progress survives closing the composer, a crash, or a restart. Images pasted from the clipboard are written to `pasted/` next to it so a draft can still find them. A local "recently viewed" list of thread / profile keys with a short text snippet lives in the same DB (`nav_history`, clearable from the History sheet). Nothing here leaves the machine; same on-disk threat model as the session file — no encryption at rest beyond FileVault.
...
1. **Updates trust one signing key.** Release builds are Developer ID-signed, notarized and stapled, and Sparkle only installs an update whose archive carries a valid EdDSA signature from the Smooblue key (`SUPublicEDKey`). Updates come from GitHub Releases over TLS. A compromise of the Sparkle private key *and* the release pipeline could ship a malicious update; custody of that key is documented in [Sparkle Updates](../Operations/Sparkle-Updates.md). Builds made locally with `scripts/bundle-macos.sh` are still ad-hoc signed.
...
6. **Bluesky DMs are NOT end-to-end encrypted** (Smooblue inherits this from the protocol). When you read or send DMs via the Messages column, requests go to your PDS with an `atproto-proxy: did:web:api.bsky.chat#bsky_chat` header that forwards them to Bluesky's chat service (`api.bsky.chat`), which stores the message bodies in plaintext on their servers. **Bluesky's operators and moderators can read your DMs** — same as Twitter/X DMs historically. The Smooblue side adds nothing on top of what `bsky.app` itself does; we're a different client talking to the same service. If you need an E2E-encrypted DM channel for sensitive conversations, use Signal / iMessage / whatever your other party is on; Bluesky DMs are appropriate for the same things you'd say in a public reply except just to one person. There is **no Smooblue setting that changes this** — the trust model is the protocol's, not the client's. Tracked: any future E2E layer would require coordination with the ATproto chat lexicon, not a Smooblue-side change.
