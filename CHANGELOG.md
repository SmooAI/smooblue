...
- [#109](https://github.com/SmooAI/smooblue/pull/109) [`f4a3e6b`](https://github.com/SmooAI/smooblue/commit/f4a3e6bc363cfdc482706da60aedcd1e98d2233c) Thanks [@brentrager](https://github.com/brentrager)! - Save posts to read later. Every post has a bookmark button next to Like; click it to save, and it fills orange. A new **Saved** column shows everything you've saved, newest first. Open it from the bookmark button in the sidebar or with `g b`. Smooblue uses Bluesky's own private bookmarks, so these are the same saves you see in the Bluesky app on your phone and on bsky.app. Save something anywhere and it shows up everywhere. Unsaving a post removes it from the Saved column right away.
...
- [#102](https://github.com/SmooAI/smooblue/pull/102) [`a1a712b`](https://github.com/SmooAI/smooblue/commit/a1a712bc47564fa9ee2bf9d5155f5c8d079d8102) Thanks [@brentrager](https://github.com/brentrager)! - Drafts and "never lose your place" navigation.

  **Drafts.** Everything you write in compose is now saved as you type, until you post it or discard it. That includes the reply or quote target, every post of a thread, and attached images and video with their alt text. You can keep many drafts at once. Closing the composer (Esc, ×, a stray click on the backdrop, or quitting the app) never loses work. A **Resume** chip above the + button brings your latest draft back. Replying to a post you already started a reply to picks up where you left off. The composer header has a **Drafts** list to switch between drafts and **New** to start another. Your old single draft is imported automatically.

  **Threads.** "+ Add post" chains posts into a thread, for new posts and replies alike. Each post gets its own character counter, up to 4 images, and clipboard paste. **Split into thread** breaks an over-long post at sentence and word boundaries. The whole thread is checked before anything is published. If a post fails partway through, the part that went out stays live and the rest is kept as a reply to it, so retrying finishes the thread instead of posting it twice.

  **Getting back to where you were.**

  - Threads and profiles have back/forward buttons (⌘[ / ⌘]).
  - ⌘⇧T, or the **Reopen** toast, brings back a thread or profile you just closed, scrolled to where you left it.
  - A new **History** panel (⌘Y, or the clock in the sidebar) lists recently viewed threads and profiles.

  Fixes found along the way:

  - A post opened from inside a profile now appears on top of it instead of hidden behind it.
  - "Continue thread →" on deep replies now opens the reply instead of doing nothing.
  - Video-only posts can be sent.
  - Esc in the @mention list closes just the list, and Esc closes the composer before the sheet under it.
  - A reply sent from an open thread now shows up in it.

- [#103](https://github.com/SmooAI/smooblue/pull/103) [`202e3f6`](https://github.com/SmooAI/smooblue/commit/202e3f623f74769b94e324a9308ce25809f515b9) Thanks [@brentrager](https://github.com/brentrager)! - Smooblue now updates itself. Release builds use Sparkle 2, the same updater as SmoothFlow. It checks for new versions every hour and shows Sparkle's standard update window: the release notes, **Skip This Version / Remind Me Later / Install Update**, and an option to install updates automatically. Choosing Install downloads the update, checks its signature, and relaunches into the new version. You can also check any time from **Smooblue → Check for Updates…** or **Settings → About**.
...
- [#84](https://github.com/SmooAI/smooblue/pull/84) [`903ce50`](https://github.com/SmooAI/smooblue/commit/903ce502685af24b15d7ff0207ce0ce6cd1960f4) Thanks [@brentrager](https://github.com/brentrager)! - Analytics pop-out layout + chart polish:
